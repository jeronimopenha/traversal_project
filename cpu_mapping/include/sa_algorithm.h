    ...
    while (T >= 0.00001)
    {
        for (int i = 0; i < sizeGrid; i++)
        {
            for (int j = i + 1; j < sizeGrid; j++)
            {
                ...
                old2 = j;
                if (node1 != INF)
                    localPositions[node1] = old2;
                if (node2 != INF)
                    localPositions[node2] = old1;
                localGrid[j] = node1;
                localGrid[i] = node2;
                ...
                { // else, undo changes and stay with previous cost
                    if (node1 != INF)
                        localPositions[node1] = old1;
                    if (node2 != INF)
                        localPositions[node2] = old2;
                    localGrid[j] = node2;
                    localGrid[i] = node1;
                }
            }
                          ...
                          unsigned int NGRIDS)
{
    ...
    for (int i = 0; i < NGRIDS; i++)
    {
        for (int j = 0; j < edges.size(); j++)
        {
            ...
            {
                if (distManhattanI == 0 && distManhattanJ == 0)
                    increment = 0;
                else
                    increment = table1hop[distManhattanI][distManhattanJ];
            }
