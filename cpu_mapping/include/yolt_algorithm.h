...
#pragma omp parallel for
    ...
    if (arch == 0)
        ...
        str_arch = "hex";
    string output = "../exp_results/placements/yolt/" + str_arch + "/" + to_string(times) + "/" + name + ".map";
    ...
    for (int t = 0; t < times; ++t)
    {
        ...
        for (int i = 0; i < EDGE_SIZE; ++i)
        {
            ...
            while (dist_x != 0 || dist_y != 0)
            {
                ...
                if (ARCH == 0 || (ARCH == 2 && pe_curr % 2 == 0))
                { // ARCH MESH OR CHESS EVEN
                    ...
                    if (change == 0)
                    { // tenta fazer uma voltinha e aumentar o caminho
                        if (neighbor_right)
                        {
                            grid_route[pe_curr][1][step] = a;
                            pos_node_y += 1;
                ...
                else if (ARCH == 1 || (ARCH == 2 && pe_curr % 2 != 0))
                { // ARCH 1-HOP OR CHESS ODD
    ...
    int *list_borders)
{
    ...
    for (int t = 0; t < times; ++t)
    {
        ...
        for (int i = 0; i < TOTAL_GRID_SIZE * __LAYERS; ++i)
            grid_place[i] = type_matrix[i];
        ...
        for (int i = 0; i < EDGE_SIZE; ++i)
        {
            ...
            if (pos_b_i == -1)
            { // verify if side 'b' already place
                // try more closed
                if (try_adjacency(a, b, pos_a_i, pos_a_j, pos_b_i, pos_b_j, dist_border,
                                  ...
                                  IN_OUT_SIZE, edges_cost_local, cost_place))
                    continue;
            }
